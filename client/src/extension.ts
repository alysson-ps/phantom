...
export async function activate(context: ExtensionContext) {
  ...
  const traceOutputChannel = window.createOutputChannel("Phantom Language Server trace");
  const command = process.env.SERVER_PATH || "phantom-language-server";
  ...
  client = new LanguageClient("phantom-language-server", "Phantom language server", serverOptions, clientOptions);
